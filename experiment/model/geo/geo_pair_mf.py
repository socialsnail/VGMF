...
from model.geo.geo_mf_grid import GeoGrid
import pickle
...
class GeoPairMatrixFactorization(BaseRecommendationModel):
    ...
    def build(self, low=-0.001, high=0.001, x_grid=0.005, y_grid=0.005):
        ...
        self.grid = GeoGrid(self.dataset)
        self.grid.grid(x_grid, y_grid)
...
if __name__ == '__main__':
    ...
    mf = GeoPairMatrixFactorization(k=10, learning_rate=0.01,
                                    lambda_=0.001, kesai=0.002,
                                    max_epoch=50, negative_rate=0.002,
                                    geo_rate=1.0)
    mf.set_dataset(data)
    mf.build(0, 0.001, 0.005, 0.005)
    mf.fit(alg='batch')
