...
class NCFDistance(object):
    ...
    def write(self, user_id, location_id, label, is_visited_before, distance):
        example = self.make_example(user_id, location_id, label, is_visited_before, distance)
        self.writer.write(example.SerializeToString())

    def make_example(self, user_id, location_id, label, is_visited_before, distance):
        example = tf.train.Example(features=tf.train.Features(
            feature={
                'user_id': _int64_feature(user_id),
                'location_id': _int64_feature(location_id),
                'label': _int64_feature(label),
                'is_visited_before': _int64_feature(is_visited_before),
                'distance': _bytes_feature(tf.compat.as_bytes(distance.tostring()))
            }
        ))
        return example


class RecPointWise(object):
    def __init__(self, file_name=None):
        self.file_name = file_name
    ...
    def write(self, user_id, location_id, label, is_visited):
        example = self.make_example(user_id, location_id, label, is_visited)
        self.writer.write(example.SerializeToString())

    def make_example(self, user_id, location_id, label, is_visited):
        example = tf.train.Example(features=tf.train.Features(
            feature={
                'user_id': _int64_feature(user_id),
                'location_id': _int64_feature(location_id),
                'label': _int64_feature(label),
                'is_visited_before': _int64_feature(is_visited)
            }
        ))
        return example


class RecDirectWriter(object):
    def __init__(self, file_name=None):
        self.file_name = file_name
    ...
    def write(self, user_id, positive_location_id, negative_location_id):
        example = self.make_example(user_id, positive_location_id, negative_location_id)
        self.writer.write(example.SerializeToString())

    def make_example(self, user_id, positive_location_id, negative_location_id):
        example = tf.train.Example(features=tf.train.Features(
            feature={
                'user_id': _int64_feature(user_id),
                'positive_location_id': _int64_feature(positive_location_id),
                'negative_location_id': _int64_feature(negative_location_id),
            }
        ))
        return example
